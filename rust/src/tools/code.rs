...
"#,
        ...
        assert!(!is_error(&result), "load_rust_code failed: {}", result_text(&result));
        ...
        assert!(json.contains("test-project"), "Project name not found: {json}");
...
"#,
        ...
        // Struct (Class)
        let json = q(&store, "SELECT ?name", "?s a code:Class ; code:name ?name .");
        ...
        // Trait
        let json = q(&store, "SELECT ?name", "?t a code:Trait ; code:name ?name .");
        assert!(json.contains("Handler"), "Trait 'Handler' not found: {json}");

        // Trait methods
        let json = q(&store, "SELECT ?method", "?t a code:Trait ; code:hasMethod ?method .");
        assert!(json.contains("handle"), "Trait method 'handle' not found: {json}");
        assert!(json.contains("name"), "Trait method 'name' not found: {json}");

        // Impl methods (linked via hasFunction)
        let json = q(&store, "SELECT ?method",
            "?c a code:Class ; code:hasFunction ?f . ?f code:name ?method .");
        assert!(json.contains("new"), "Impl method 'new' not found: {json}");

        // Import
        let json = q(&store, "SELECT ?path", "?i a code:Import ; code:importPath ?path .");
        assert!(json.contains("HashMap"), "Import not found: {json}");

        // Module declaration
        let json = q(&store, "SELECT ?name", r#"?m a code:Module ; code:name ?name . FILTER(?name != "")"#);
        assert!(json.contains("utils"), "Module 'utils' not found: {json}");

        // Docstring
        let json = q(&store, "SELECT ?doc",
    ...
    fn test_directory_loading_with_file_discovery() {
        ...
        assert!(text.contains("2 file(s)"), "Expected 2 files loaded: {text}");

        let json = q(&store, "SELECT ?path", "?m a code:Module ; code:relativePath ?path .");
        assert!(!json.contains("target"), "target/ should be ignored: {json}");
    }
    ...
    fn test_auto_detection() {
        ...
        assert!(!is_error(&result), "Auto-detect failed: {}", result_text(&result));
        assert!(
    ...
    fn test_single_file_loading() {
        ...
        let json = q(&store, "SELECT ?name", "?f a code:Function ; code:name ?name .");
    ...
    fn test_custom_graph() {
        ...
        assert!(json.contains("custom_fn"), "Function not in custom graph: {json}");
    }
